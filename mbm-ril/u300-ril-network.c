...
void onRegistrationStatusChanged(const char *s)
{
    ...
    if (update) {
        ...
        LOGW("%s() Skipping unsolicited response since no change in state", __func__);
...
error:
    LOGE("%s() Unable to parse (%s)", __func__, s);
    goto finally;
}
...
char *getNetworkType(int def)
{
    ...
    if (s_umts_rinfo > ERINFO_UMTS_NO_UMTS_HSDPA) {
        ...
        LOGD("%s() Using 2G info: %d", __func__, s_gsm_rinfo);
        if (s_gsm_rinfo == 1)
...
void requestOperator(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (!(s_reg_change)) {
        if (old_response[0] != NULL) {
            ...
            LOGW("%s() Using buffered info since no change in state", __func__);
            goto no_sim;
        }
    }
